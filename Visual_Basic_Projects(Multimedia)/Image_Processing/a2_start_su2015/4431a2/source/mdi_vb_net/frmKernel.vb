...
#End Region
    ...
    Overrides Function Process(ByRef image As Bitmap, ByVal x As Integer, ByVal y As Integer) As Color
        ...
        Dim size As Integer = kernel_width * kernel_height
        ...
        Dim kernel_j As Integer
        Dim kernel_i As Integer = 0

        For i = x - cx To x + cx
            kernel_j = 0
            If (kernel_width Mod 2 = 0) And (i = x + cx) Then
                Exit For
            End If

            For j = y - cy To y + cy
                If (kernel_height Mod 2 = 0) And (j = y + cy) Then
                    Exit For
                End If

                pixColor = GetPixel(image, i, j)
                red = red + pixColor.R * kernel(kernel_i, kernel_j)
                green = green + pixColor.G * kernel(kernel_i, kernel_j)
                blue = blue + pixColor.B * kernel(kernel_i, kernel_j)
